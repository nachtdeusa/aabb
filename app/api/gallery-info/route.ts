...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const thumbnail = await findThumbnailInFolder(folderPath)

    // Determinar o tipo de pasta
    const type = await determineFolderType(folderPath)
    ...
    return NextResponse.json({ error: "Erro ao obter informações da galeria" }, { status: 500 })
  }
}
